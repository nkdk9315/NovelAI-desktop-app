...
`src/types/index.ts` に追加された Tag DB 固有の TS 型:

| 型 | フィールド | 説明 |
|---|---|---|
| `TagDto` | `id`, `name`, `csvCategory` | タグ本体 |
| `TagGroupDto` | `id`, `slug`, `title`, `parentId`, `kind`, `source`, `sortKey`, `childCount`, `isFavorite` | タググループ（`isFavorite` は migration 014） |
| `TagWithGroupsDto` | `tag: TagDto`, `groups: TagGroupDto[]` | グローバル検索結果（タグ＋所属グループ） |
| `CountByIdDto` | `id`, `count` | グループ別メンバー/子孫カウント用 |
...
マルチキャラクター対話プリセット機能。プリセットは複数のキャラクタースロットを持ち、サイドバーグループインスタンスがプロジェクト×キャラペアにバインドする。

#### `prompt_presets` (migration 022, 026)

| フィールド | 型 | 説明 |
|---|---|---|
| `id` | TEXT PK | UUID |
| `name` | TEXT NOT NULL | プリセット名（trim 後空禁止、最大 255 文字） |
| `folder_id` | INTEGER NULL | FK → `preset_folders(id)` ON DELETE SET NULL |
| `sort_key` | INTEGER NOT NULL DEFAULT 0 | フォルダ内表示順（migration 026）。ドラッグ並び替えで更新される |
| `created_at` / `updated_at` | TEXT | ISO 8601 |

インデックス: `idx_prompt_presets_folder_sort (folder_id, sort_key)`。`list` は `ORDER BY sort_key ASC, created_at DESC`。

#### `preset_character_slots` (migration 022 + 025)

| フィールド | 型 | 説明 |
|---|---|---|
| `id` | TEXT PK | UUID |
| `preset_id` | TEXT FK | → `prompt_presets(id)` ON DELETE CASCADE |
| `slot_index` | INTEGER | 0-based 表示順 |
| `slot_label` | TEXT | 表示ラベル |
| `genre_id` | TEXT NULL | FK → `genres(id)` ON DELETE SET NULL |
| `positive_prompt` / `negative_prompt` | TEXT | プロンプト本文 |
| `role` | TEXT | `"target"` / `"source"` / `"none"` |
| `position_x` / `position_y` | REAL | キャンバス上座標 `0.0..=1.0`、DEFAULT 0.5（025） |

#### `preset_folders` (migration 022)

| フィールド | 型 | 説明 |
|---|---|---|
| `id` | INTEGER PK AUTOINCREMENT | |
| `title` | TEXT NOT NULL | |
| `parent_id` | INTEGER NULL | FK → `preset_folders(id)` ON DELETE CASCADE（自己参照で階層化） |
| `sort_key` | INTEGER DEFAULT 0 | 兄弟間表示順 |

`move_folder` は親方向への循環参照を検出して拒否する。

#### `sidebar_preset_group_instances` (migration 023 + 024)

| フィールド | 型 | 説明 |
|---|---|---|
| `id` | TEXT PK | UUID |
| `project_id` | TEXT FK | → `projects(id)` ON DELETE CASCADE |
| `folder_id` | INTEGER FK | → `preset_folders(id)` ON DELETE CASCADE |
| `source_character_id` | TEXT | フロントエンドのキャラ UUID、FK 非強制 |
| `target_character_id` | TEXT | 同上（`source != target` バリデーション） |
| `position` | INTEGER | サイドバー並び順 |
| `default_positive_strength` / `default_negative_strength` | REAL DEFAULT 1.0 | `1.0..=10.0`（024） |
| `created_at` / `updated_at` | TEXT | ISO 8601 |

キャラクター ID が FK でない理由: キャラクターデータが `GenerateParams` の JSON として保存されており、独立テーブル化されていないため。

#### `sidebar_preset_group_active_presets` (migration 023 + 024 + 025)

インスタンスごとに有効なプリセットを管理する中間テーブル。

| フィールド | 型 | 説明 |
|---|---|---|
| `instance_id` | TEXT FK | → `sidebar_preset_group_instances(id)` ON DELETE CASCADE |
| `preset_id` | TEXT FK | → `prompt_presets(id)` ON DELETE CASCADE |
| `positive_strength` | REAL NULL | per-preset 上書き（NULL = インスタンス default 継承）（024） |
| `negative_strength` | REAL NULL | 同上 |
| `activated_at` | TEXT | アクティベーション時刻、last-wins による位置情報解決に使用（025） |

複合 PK: `(instance_id, preset_id)`。
