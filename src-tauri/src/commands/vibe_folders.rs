...
use crate::services::vibe_folder as svc;
...
#[tauri::command]
pub fn list_vibe_folder_children(
    state: State<'_, AppState>,
    parent_id: i64,
) -> Result<Vec<AssetFolderDto>, String> {
    let conn = lock_db(&state)?;
...
#[tauri::command]
pub fn delete_vibe_folder(state: State<'_, AppState>, folder_id: i64) -> Result<(), String> {
    let conn = lock_db(&state)?;
...
pub fn set_vibe_folder(
    ...
    let conn = lock_db(&state)?;
    svc::set_vibe_folder(&conn, &vibe_id, folder_id).map_err(Into::into)
}

#[tauri::command]
pub fn count_vibes_per_folder(
    state: State<'_, AppState>,
) -> Result<Vec<CountByIdDto>, String> {
    let conn = lock_db(&state)?;
    svc::count_vibes_per_folder(&conn).map_err(Into::into)
}
