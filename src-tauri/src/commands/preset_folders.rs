...
use crate::services::preset_folder as svc;
...
#[tauri::command]
pub fn delete_preset_folder(state: State<'_, AppState>, folder_id: i64) -> Result<(), String> {
    let conn = lock_db(&state)?;
    svc::delete(&conn, folder_id).map_err(Into::into)
}

#[tauri::command]
pub fn delete_presets_in_folder(
    state: State<'_, AppState>,
    folder_id: i64,
) -> Result<usize, String> {
    let conn = lock_db(&state)?;
    svc::delete_presets_in_folder(&conn, folder_id).map_err(Into::into)
}

#[tauri::command]
pub fn count_presets_in_folder(
    state: State<'_, AppState>,
    folder_id: i64,
) -> Result<i64, String> {
    let conn = lock_db(&state)?;
    svc::count_presets_in_subtree(&conn, folder_id).map_err(Into::into)
}
