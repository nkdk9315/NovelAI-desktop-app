...
モジュール数の正準は `src-tauri/src/{commands,services,repositories}/mod.rs` および `src/{stores,lib}`。詳細な責務分担は `docs/architecture/ARCHITECTURE.md` §4 を参照。
...
- **AppState**: `Mutex<Connection>` + `tokio::sync::Mutex<Option<NovelAIClient>>` + `SystemPromptDB` + `PathBuf(app_data_dir)`
